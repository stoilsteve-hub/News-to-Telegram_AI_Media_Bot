...
def openai_strict_blocks(client: OpenAI, source: str, title: str, rss_summary: str, link: str, article_type: str) -> str:
    ...
    prompt = f"""
Ты — профессиональный журналист-редактор Telegram-канала о жизни в Швеции. Твоя цель — превратить сухой текст в яркий, глубоко информативный и захватывающий пост.

ПРАВИЛА КОНТЕНТА:
- Используй ТОЛЬКО факты из предоставленного текста.
- ЗАПРЕЩЕНО использовать вводные фразы типа "Статья обсуждает...", "В тексте говорится...", "Автор считает...". Сразу переходи к делу.
- НАЙДИ "КРЮЧОК": Ищи самые интересные детали, необычные сравнения, яркие цитаты или конкретные цифры (например, "переливание крови" или "теннисный кросс" из статьи про Шекспира).
- ПИШИ КОНКРЕТНО: Имена, должности, точные суммы, даты, названия ведомств.
- СТИЛЬ: Живой, журналистский, ориентированный на пользу и любопытство читателя. Почему это важно? Что в этом удивительного?
- ЯЗЫК: Только русский (кириллица). Никаких латинских символов.

СТРУКТУРА:
1. HEADLINE: Максимально цепляющий заголовок с 1 эмодзи в начале.
2. SUMMARY: Суть истории в 2-4 предложениях. Без воды.
3. DETAILS: Самые важные и мелкие подробности списком.
4. HASHTAGS: Темы поста.
...
def openai_translate_compose(client: OpenAI, title: str, rss_summary: str, article_type: str) -> tuple[str, str, str, list[str]]:
    ...
    prompt = f"""
Создай профессиональный журналистский пост для Telegram на основе новости.
Язык: Только русский (кириллица). Без английских/шведских слов.

ТРЕБОВАНИЯ К ТЕКСТУ:
- НИКАКИХ фраз типа "Текст рассказывает..." или "В статье говорится...". Сразу к фактам.
- Найди детали, которые зацепят читателя: конкретные цифры, имена, необычные факты.
- Сделай текст "выпуклым" и живым, а не просто пересказом.
...
Сделай:
1) ЗАГОЛОВОК (1 строка) — начни с одного эмодзи. Должен интриговать.
2) РЕЗЮМЕ (2–4 предложения, 70–110 слов). Самая суть и "почему это важно".
3) ДЕТАЛИ (8–12 коротких строк): Фактология, цитаты, подробности.
